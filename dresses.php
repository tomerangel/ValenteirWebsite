...
$query="SELECT * FROM product WHERE size='All' and catagory='dresses'";
...
while ($row =mysqli_fetch_array($run))
{
	...
	<?php if(isset($_POST['submit']) and isset($_SESSION['username']))
	{
		...
		if(mysqli_num_rows($res1)!=0)
			echo "<script>alert('The product choosen is already in the cart!'); window.location.href='dresses.php'</script>";
