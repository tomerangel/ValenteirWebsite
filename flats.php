...
<center>
    <table border=1>
        <h2><th colspan=8>נעליי שטוח</th></h2>
        <tr><th>שם:</th>
            <th>תמונה</th>
            <th>מחיר</th>
            <th>גודל</th>
...
$query="SELECT * FROM product WHERE size='All' and catagory='flats'";
...
while ($row =mysqli_fetch_array($run))
{
	...
	<?php if(isset($_POST['submit']) and isset($_SESSION['username']))
	{
		...
		if(mysqli_num_rows($res1)!=0)
			echo "<script>alert('The product choosen is already in the cart!'); window.location.href='flats.php'</script>";

		else if($res<1)
			echo "<script>alert('The product choosen is out of Stock!'); window.location.href='flats.php'</script>";
		else
		{
