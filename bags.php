...
$query="SELECT * FROM product WHERE size='All' and catagory='bags'";
...
while ($row =mysqli_fetch_array($run))
{
	...
	<?php if(isset($_POST['submit']) and isset($_SESSION['username']))
	{
		...
		if(mysqli_num_rows($res1)!=0)
			echo "<script>alert('בחרת כבר את המוצר הזה'); window.location.href='bags.php'</script>";

		else if($res<1)
			echo "<script>alert('נגמר כמות המוצר'); window.location.href='bags.php'</script>";
		else
		{
	...
	while($row=mysqli_fetch_array($run))
		{
			...
			if (mysqli_query($db,$query))
				echo "<script type='text/javascript'>alert('המוצר נוסף בהצלחה')</script>";
			else
				echo "<script type='text/javascript'>alert('הוספת המוצר נכשל נסה שוב')</script>";
		}
	}
	?>
	<?php if(isset($_POST['submit']) and (!isset($_SESSION['username'])))
