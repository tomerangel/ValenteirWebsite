...
<?php
	...
	if(isset($_POST['purchase']))
	{
		...
		if(strlen($cardnum)<10 or (strlen($cardnum))>20  )
		{
		...
		if (mysqli_query($db,$query))
		{
			...
			$run=mysqli_query($db,$query) or die('Sql Error');
			...
			if($query)
		{
			$res=mysqli_query($db1,$query) or die('Sql Error');
			...
			mysqli_query($db1,$delete) or die('Sql Error');
			unset($_SESSION['ptotal']);
			echo "<script>alert('Purchase succesfully'); window.location.href='home.php'</script>";
		}
		else
		{
			echo "<script>alert('Purchase Failed!'); window.location.href='home.php'</script>";
		}
		}
		}
	}?>
